  ...
  try {
    ...
    if (!fs.existsSync(filePath) && fs.existsSync(circuitsDir)) {
      ...
      for (const yDir of yearDirs) {
        const candidatePath = path.join(circuitsDir, yDir, `${circuitId}.json`);
    ...
    if (!circuitData) {
      const trackDef = TRACKS_REGISTRY[circuitId];
      ...
      if (fs.existsSync(heritageFilePath)) {
        try {
          ...
          const matchedCircuit = hJson.circuits?.find((c: any) => c.id === circuitId);
          if (matchedCircuit) {
    ...
    const heritageFilePath = path.join(process.cwd(), 'paddock_f1_2026_circuit_corners.json');
    if (fs.existsSync(heritageFilePath) && Array.isArray(circuitData.corners)) {
      try {
        const heritageContent = fs.readFileSync(heritageFilePath, 'utf8');
        const heritageJson = JSON.parse(heritageContent);
        const circuitHeritage = heritageJson.circuits?.find((c: any) => c.id === circuitId);
